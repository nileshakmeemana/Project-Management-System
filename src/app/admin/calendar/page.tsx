...
interface Deadline{id:number;title:string;date:string;color:string;employee:string;}

const DEFAULT:Deadline[]=[
  {id:1,title:'Nail Toolz — banner delivery',date:'2026-06-18',color:'#c70a24',employee:'Gayani Diaz'},
  {id:2,title:'ANVAYA — reels batch',date:'2026-06-22',color:'#005bd3',employee:'Gayani Diaz'},
  {id:3,title:'Second Page — blog post',date:'2026-06-25',color:'#047b5d',employee:'Sara Kim'},
  {id:4,title:'Port Stephens — email campaign',date:'2026-07-01',color:'#956f00',employee:'Gayani Diaz'},
];
...
export default function AdminCalendarPage() {
  ...
  const [deadlines,setDeadlines]=useState<Deadline[]>(DEFAULT);
  const [dlId,setDlId]=useState(10);
  const [modal,setModal]=useState(false);
  const [editId,setEditId]=useState<number|null>(null);
  const [form,setForm]=useState({title:'',date:'',color:COLORS[0],employee:''});

  const prev=()=>{if(month===0){setMonth(11);setYear(y=>y-1);}else setMonth(m=>m-1);};
  const next=()=>{if(month===11){setMonth(0);setYear(y=>y+1);}else setMonth(m=>m+1);};
  const openModal=(id?:number)=>{const d=id?deadlines.find(x=>x.id===id):null;setEditId(id||null);setForm(d?{title:d.title,date:d.date,color:d.color,employee:d.employee}:{title:'',date:'',color:COLORS[0],employee:''});setModal(true);};
  const saveModal=()=>{if(!form.title||!form.date)return;if(editId)setDeadlines(dl=>dl.map(d=>d.id===editId?{...d,...form}:d));else{setDeadlines(dl=>[...dl,{id:dlId,...form}]);setDlId(i=>i+1);}setModal(false);};
  ...
  deadlines.forEach(d=>{const dd=new Date(d.date);if(dd.getFullYear()===year&&dd.getMonth()===month){const k=dd.getDate();if(!evMap[k])evMap[k]=[];evMap[k].push(d);}});
  ...
  return (
    <div className="content">
      ...
      <div className="cal-page-grid">
        <div className="p-card">
          ...
          <div className="cal-big-grid">
            ...
            {Array.from({length:dim},(_,i)=>{const d=i+1;return(<div key={d} className={`cal-big-cell${isToday(d)?' today':''}`}><span className="cal-day-num">{d}</span>{(evMap[d]||[]).map(e=><div key={e.id} className="cal-ev-pill" style={{background:e.color}} onClick={()=>openModal(e.id)}>{e.title}</div>)}</div>);})}
              ...
              {upcoming.map(d=>{const diff=Math.round((new Date(d.date).getTime()-new Date(now.toDateString()).getTime())/864e5);return(
                <div key={d.id} style={{display:'flex',alignItems:'center',gap:'var(--p-space-300)',padding:'var(--p-space-300) var(--p-space-400)',borderBottom:'.0625rem solid var(--p-border-subdued)'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:'var(--p-font-size-275)',color:'var(--p-text-secondary)'}}>{d.employee||'—'} · {new Date(d.date).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</div>
                  </div>
      ...
      {modal&&(<div className="p-modal-bg open" onClick={()=>setModal(false)}><div className="p-modal" style={{maxWidth:420}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="p-modal-body">
          ...
          <div className="p-grid2">
            ...
            <div className="p-field"><label className="p-label">Employee</label><input className="p-input" value={form.employee} onChange={e=>setForm(f=>({...f,employee:e.target.value}))} placeholder="Employee name"/></div>
          </div>
        ...
        <div className="p-modal-ft">{editId&&<button className="btn-secondary" style={{color:'var(--p-text-critical)',marginRight:'auto'}} onClick={()=>{remove(editId);setModal(false);}}>Delete</button>}<button className="btn-secondary" onClick={()=>setModal(false)}>Cancel</button><button className="btn-primary" onClick={saveModal}>{editId?'Save changes':'Add deadline'}</button></div>
      </div></div>)}
    </div>
  );
}
