...
export default function ActivityLogPage() {
  ...
  const fetchLogs=async()=>{
    try{const params=new URLSearchParams();params.set('limit','500');if(filter)params.set('category',filter);const d=await apiCall('GET',`/activity?${params}`);setPage(1);setLogs(d.logs||[]);}
  ...
  return (
    <div className="content">
      ...
      <div className="p-table-wrap">
        ...
        <div className="p-table-footer">{paginated.length} log entr{filtered.length!==1?'ies':'y'}</div>
        </div>
    </div>
  );
}
